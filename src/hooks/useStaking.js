...
const cloudaxTokenAddress = process.env.REACT_APP_CLOUDAX_ADDRESS || null;
...
export default function useStaking({ address, ABI, userAddress }) {
    ...
    const checkpoints = {
        stake: {
            ...
            "Amount would overflow maximum stake": "Amount would overflow maximum stake",
    ...
    const stake = async (amount) => {
        ...
        try {
            const approveTrx = await cloudaxToken.connect(web3.getSigner()).approve(staker.address, etherAmount);
    ...
    React.useEffect(() => {
        if (staking) {
            const { "0": stakersCount, "1": totalStaked, "2": maxStake, "3": rewardsPerHour, "4": minStake, "5": stakingIsOpen } = staking;
    ...
    return { stakingInfo, userStake, stake, unstake, loadStakingInfo, getUserStake }
} 